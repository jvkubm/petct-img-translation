...
class UNet2D(nn.Module):
    def __init__(self, in_channels, out_channels, init_features=32):
        ...
        self.encoder2 = UNet2D._block(features, features * 2)
        self.encoder3 = UNet2D._block(features * 2, features * 4)
        self.encoder4 = UNet2D._block(features * 4, features * 8)
        # Pooling
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
        
        # Bottleneck
        self.bottleneck = UNet2D._block(features * 8, features * 16)
        
        # Decoder path
        self.upconv4 = nn.ConvTranspose2d(features * 16, features * 8, kernel_size=2, stride=2)
        self.decoder4 = UNet2D._block((features * 8) * 2, features * 8)
        self.upconv3 = nn.ConvTranspose2d(features * 8, features * 4, kernel_size=2, stride=2)
        self.decoder3 = UNet2D._block((features * 4) * 2, features * 4)
        self.upconv2 = nn.ConvTranspose2d(features * 4, features * 2, kernel_size=2, stride=2)
        self.decoder2 = UNet2D._block((features * 2) * 2, features * 2)
    ...
    @staticmethod
    def _block(in_channels, features, name=""):
        # Convolutional block with Conv2D, BatchNorm2D, and ReLU
        return nn.Sequential(
